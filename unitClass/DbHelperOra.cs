...
{
    /// <summary>
    /// 数据访问抽象基础类 2014-10-16 付辉平
    /// </summary>
	public class DbHelperOra
	{
        //数据库连接字符串(web.config来配置)，可以动态更改connectionString支持多数据库.		
        public static string connectionString = ConfigurationManager.ConnectionStrings["ConnectionStringTool"].ConnectionString;
		public DbHelperOra()
		{			
		}
        ...
        public static void LoginLog(string UserName, string StrIP, string MoudelName)
        {
            string strsql = "select count(LOGINIP) from wap_log where MOUDLENAME = '" + MoudelName + "' and LOGINIP = '" + StrIP + "' and LOGINDATE = '" + DateTime.Now.ToString("yyyy-MM-dd") + "'";
            if (!Exists(strsql))
            {
                strsql = "insert into wap_log values('" + UserName + "','" + DateTime.Now.ToString("yyyy-MM-dd") + "','" + StrIP + "','" + MoudelName + "')";
                ExecuteSql(strsql);
            }
        }
